...
| 용어 | 풀이 | 설명 |
|------|------|------|
| AUP | Acceptable Use Policy | 허용 가능한 사용 정책 |
| DAC | Discretionary Access Control | 임의적 접근 제어. 소유자가 권한 결정 |
| MAC | Mandatory Access Control | 강제적 접근 제어. 시스템이 권한 결정 |
| RBAC | Role-Based Access Control | 역할 기반 접근 제어 |
| ABAC | Attribute-Based Access Control | 속성 기반 접근 제어 |
| FIM | File Integrity Monitoring | 파일 무결성 모니터링 |
| AIDE | Advanced Intrusion Detection Environment | Linux 파일 무결성 검사 도구 |
| Tripwire | - | 파일 무결성 모니터링 솔루션 |
| DLP | Data Loss Prevention | 데이터 유출 방지 |
| WSUS | Windows Server Update Services | Windows 업데이트 중앙 관리 |
| SCCM/MECM | Microsoft Endpoint Configuration Manager | Microsoft 엔드포인트 관리 도구 |
| Nmap | Network Mapper | 네트워크 스캐닝 및 포트 탐지 도구 |
| Nessus | - | 상용 취약점 스캐너 (Tenable) |
| OpenVAS | Open Vulnerability Assessment Scanner | 오픈소스 취약점 스캐너 |
| Nikto | - | 웹 서버 취약점 스캐너 |
| OWASP ZAP | Zed Attack Proxy | 오픈소스 웹 취약점 스캐너 |
| Burp Suite | - | 웹 프록시 및 취약점 분석 도구 |
| SQLmap | - | SQL Injection 자동화 도구 |
| Metasploit | - | 침투 테스트 프레임워크 |
| Meterpreter | - | Metasploit의 고급 페이로드 셸 |
| John the Ripper | - | 패스워드 크래킹 도구 |
| Hashcat | - | GPU 가속 패스워드 크래킹 도구 |
| Hydra | - | 온라인 패스워드 무차별 대입 도구 |
| Wireshark | - | 네트워크 패킷 분석기 |
| tcpdump | - | CLI 패킷 캡처 도구 |
| Volatility | - | 메모리 포렌식 프레임워크 |
| Autopsy | - | GUI 디스크 포렌식 플랫폼 |
| Sleuth Kit | - | CLI 포렌식 도구 모음 |
| EDR | Endpoint Detection and Response | 엔드포인트 위협 탐지 및 대응 |
| EPP | Endpoint Protection Platform | 엔드포인트 보호 플랫폼 (예방 중심) |
...
| IPS | Intrusion Prevention System | 침입 방지 시스템 (탐지 + 차단) |
| Snort | - | 오픈소스 IDS/IPS |
| Suricata | - | 고성능 오픈소스 IDS/IPS |
| NGFW | Next-Generation Firewall | 차세대 방화벽. DPI, IPS, 앱 인식 |
| WAF | Web Application Firewall | 웹 애플리케이션 방화벽 |
| ModSecurity | - | 오픈소스 WAF |
| NAC | Network Access Control | 네트워크 접근 제어 |
| 802.1X | - | 포트 기반 네트워크 접근 제어 표준 |
...
| XDR | Extended Detection and Response | 확장 탐지 및 대응. EDR + 네트워크 + 클라우드 |
| RPO | Recovery Point Objective | 복구 목표 시점. 허용 가능한 데이터 손실량 |
| RTO | Recovery Time Objective | 복구 목표 시간. 서비스 복구까지 허용 시간 |
| MTTR | Mean Time To Recover | 평균 복구 시간 |
| MTBF | Mean Time Between Failures | 평균 고장 간격 |
| VSS | Volume Shadow Copy Service | Windows 볼륨 스냅샷 서비스 |
