...
```mermaid
flowchart TB
    Policy["접근통제 정책<br/>(어떤 규칙으로 통제할 것인가)"]
    Model["접근통제 모델<br/>(규칙의 표현)"]
    Mechanism["접근통제 메커니즘<br/>(규칙의 구현)"]
...
```mermaid
flowchart LR
    subgraph ID["식별 (Identification)"]
        ID_DESC["나는 누구<br/>(ID 제시)"]
    end

    subgraph AUTH["인증 (Authentication)"]
        AUTH_DESC["본인이 맞는지<br/>(비밀번호 확인)"]
    end

    subgraph AUTHZ["인가 (Authorization)"]
        AUTHZ_DESC["무엇을 할 수 있는지<br/>(권한 확인)"]
    end

    subgraph ACC["책임추적 (Accountability)"]
        ACC_DESC["누가 무엇을 했는지<br/>(로그 기록)"]
    end
...
```mermaid
xychart-beta
    title "FAR/FRR과 CER/EER 관계"
    x-axis "임계값" [낮음, "", "", 중간, "", "", 높음]
    y-axis "에러율" 0 --> 100
    line "FRR" [90, 70, 50, 35, 20, 10, 5]
    line "FAR" [5, 10, 20, 35, 50, 70, 90]
```

> CER/EER: FAR과 FRR이 교차하는 지점 (중간 부근)
...
sequenceDiagram
    ...
    C->>KDC: 1. AS-REQ (ID, TGS ID)
    KDC->>C: 2. AS-REP (TGT, 세션키)
    C->>KDC: 3. TGS-REQ (TGT, 서비스 ID)
    KDC->>C: 4. TGS-REP (서비스 티켓)
    C->>S: 5. AP-REQ (서비스 티켓)
    S->>C: 6. AP-REP (인증 완료)
```
...
```mermaid
flowchart LR
    Users["사용자<br/>(Users)"] --> Roles["역할<br/>(Roles)"] --> Permissions["권한<br/>(Permissions)"]
```
